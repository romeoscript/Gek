...
// Function to create 70% frame manifest
async function create70PercentFrameManifest() {
  console.log('🚀 Creating 70% frame manifest for optimal quality/performance balance...\n');
  ...
  Object.keys(folders).forEach(folderName => {
    ...
    if (files.length > 0) {
      // Calculate 70% of frames
      const targetFrameCount = Math.round(files.length * 0.7);
      
      // Calculate step to get approximately 70% of frames
      const step = Math.max(1, Math.round(files.length / targetFrameCount));
      
      // Select frames to get close to 70%
      ...
      // If we have fewer frames than 70%, add some more
      if (selectedFrames.length < targetFrameCount) {
  ...
  // Save 70% manifest
  const manifestPath = path.join(__dirname, 'public', 'animations', 'manifest-70percent.json');
  ...
  console.log(`\n✅ 70% manifest saved to: ${manifestPath}`);
  console.log('\n📊 Frame summary:');
  console.log(`   Original frames: ${originalFrames}`);
  console.log(`   70% frames: ${totalFrames}`);
...
if (cloudinary.v2.config().cloud_name === 'YOUR_CLOUD_NAME') {
  ...
  create70PercentFrameManifest().catch(console.error);
}
