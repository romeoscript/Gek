...
// Ultra-lightweight texture loader
class LightweightTextureLoader {
    constructor() {
        this.cache = new Map()
        this.maxCacheSize = 10 // Very small cache for testing
    }
...
class LightweightSequencePlayer {
    constructor(params) {
        ...
        this.current = null
        this.textureLoader = new LightweightTextureLoader()
    ...
    async loadSequence(key) {
        ...
        console.log(`🎬 Loading lightweight sequence: ${key} with ${frames.length} frames`)
        
        // Load all frames immediately for lightweight version
        await Promise.all(frames.map(url => this._loadTexture(url)))
...
class GEKLanding {
    ...
    async loadManifest() {
        // Use test manifest for lightweight version
        const res = await fetch('/animations/manifest-test.json')
        this.manifest = await res.json()
        console.log('✅ Loaded lightweight test manifest')
    }
    ...
    async _createBackgroundPlayer() {
        const player = new LightweightSequencePlayer({
            scene: this.scene,
            camera: this.camera,
